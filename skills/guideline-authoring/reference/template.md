...
節の順序は変えない（目的 → スコープ → 前提 → 原則 → フロー → 軸 → エスカレーション → 成果物 → 適用例（任意）→ ケーススタディ → 参考）。目的とスコープを先に読ませないと、手順だけが参照されて例外時に判断できなくなる。
...
<ステップが多い作業ガイドのときに置く。架空でよいので、入口から成果物まで通した例。表が埋まった状態を見せて粒度を伝える。>
